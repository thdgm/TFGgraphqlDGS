...
external interface FilterFormDataSeriesProps:Props{
    var filterList: Collection<SeriesModel>
    var handleOnChange: (event: ChangeEvent<HTMLElement>) -> Unit
    var addList: Collection<String>

}

val filterFormDataSeries = FC<FilterFormDataSeriesProps> {props->

    var filtersTypesFields by useState(mutableListOf<Collection<String>>(listOf(),listOf(),listOf()))
    var dataSeriesList by useState(props.filterList)

    useEffect(emptyList<Collection<String>>()) {
        val coroutineScope = CoroutineScope(Dispatchers.Default)
        coroutineScope.launch {
            val publishers = components.commmon.pages.catalog.mainPage.getAllPublishers(true)
            val adminLevels = components.commmon.pages.catalog.mainPage.getAllAdminLevels(true)
            filtersTypesFields = mutableListOf(publishers,
                adminLevels,
                listOf("Ningún dataset","Menos de 100", "Entre 100 y 1000", "Entre 1000 y 10.000", "Más de 10.000")
            )
        }
    }
