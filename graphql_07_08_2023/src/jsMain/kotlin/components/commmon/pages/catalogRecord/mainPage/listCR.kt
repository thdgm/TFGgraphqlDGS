...
import components.commmon.searcher.SearcherCR
import components.commmon.searcher.SearcherCatalogs
import components.commmon.selectFilter.selectedCatalogRecord.selectFilterCR
...
external interface ListCRProps: Props {
    var searchBy: String
    var cRList: Collection<CatalogRecordModel>
    var numberOfCR: Int
}
...
val listCR = FC<ListCRProps> { props ->
    ...
    val handleOnClick: (event: MouseEvent<HTMLElement, *>) -> Unit = { event->
        navigate("/catalogrecords/${event.currentTarget.id}")
    }
    ...
    Paper {
        ...
        if (cRList.isEmpty()) { //Esto durante 4 segundos, luego muestra no se ha encontardo nada
            ...
            isDisabled = false
            mui.material.List {
                cRList.filter { if (props.searchBy.isNotEmpty()) it.title!!.contains(props.searchBy) else true }
                    .map {
                        ListItemButton {
                            onClick = handleOnClick

                            id = it.id
                            CardListCatalogRecord { this.crInfo = it }
                        }
                    }
            }
            Pagination{
                numberOfPages = props.numberOfCR
                resType = "CatalogRecords"
            }
        }
