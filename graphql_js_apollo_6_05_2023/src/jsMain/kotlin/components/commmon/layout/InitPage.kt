...
suspend fun getDatasets(filter: String?, values: List<String>?, page: Int): List<DatasetModel>{
    val l =  components.commmon.filterForm.apolloClient.query(DatasetsQuery(filter=Optional.present(filter), value=Optional.present(values), page = page)).execute().data?.resourcesByFilter?.filterNotNull() ?: emptyList()
