...
suspend fun getDatasets(filter: String?, values: List<String>?): List<DatasetModel>{
    val l =  components.commmon.filterForm.apolloClient.query(DatasetsQuery(filter=Optional.present(filter), value=Optional.present(values), page = 0)).execute().data?.resourcesByFilter?.filterNotNull() ?: emptyList()
