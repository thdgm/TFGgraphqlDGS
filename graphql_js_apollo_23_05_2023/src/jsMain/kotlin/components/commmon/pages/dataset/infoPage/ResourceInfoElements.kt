            ...
            Paper {
                ...
                List {
                    datasetInfo.elementAt(0)?.servedByID?.mapIndexed { i, it ->
                        ...
                        val valueToShow = if (!datasetInfo.elementAt(0)?.servedBytIdentifiers?.elementAt(i).isNullOrEmpty()){
            ...
            Paper {
                ...
                List {
                    datasetInfo.elementAt(0)?.inSeriesID?.mapIndexed { i, it ->
                        ...
                        val valueToShow = if (!datasetInfo.elementAt(0)?.inSeriesIdentifiers?.elementAt(i).isNullOrEmpty()){
            ...
            Paper {
                ...
                List {
                    datasetInfo.elementAt(0)?.inCatalogID?.mapIndexed { i, it ->
                        ...
                        val valueToShow = if (!datasetInfo.elementAt(0)?.inCatalogIdentifiers?.elementAt(i).isNullOrEmpty()){
                               ...
                               addToFilters = {
                                   ...
                                   selectedFilters =
                                       selectedFilters.toMutableMap().mapValues { (key, catalogMap) ->
                                           if (key == "Datasets") {
                                               catalogMap!!.toMutableMap().mapValues { (innerKey, filterVal) ->
                                                   if (innerKey == "Distribuciones" && !filterVal.contains(valueToShow)) filterVal.plus(
                                                       valueToShow
                                                   )
                                                   else if (filterVal.contains(valueToShow)) filterVal.filter { miVal -> miVal != valueToShow }
                                                   else filterVal
                        ...
                        Chip {
                            ...
                            onClick = {
                                selectedFilters = selectedFilters.toMutableMap().mapValues { (key, catalogMap) ->
                                    if (key == "Datasets") {
                                        catalogMap!!.toMutableMap().mapValues { (innerKey, filterVal) ->
                                            if (innerKey == "Etiqueta" && !filterVal.contains(chipValue)) filterVal.plus(chipValue)
                                            else if (filterVal.contains(chipValue)) filterVal.filter { miVal -> miVal != chipValue }
                                            else filterVal
                                        }.toMutableMap()
                                    } else {
