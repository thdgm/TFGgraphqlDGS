...
class DatasetTest {
    ...
    fun `el dataset d1 tiene 4 keywords`() {
        val keywords: Collection<String> = dgsQueryExecutor.executeAndExtractJsonPath(
            ...
            """.trimIndent(),
            "data.dataset.keywords[*].literal"
        )
        getLogger("loggerTest").debug("$keywords")
    ...
    fun `el dataset d1 tiene un temporal correcto end mayor que start`() {
        ...
        val start = convertersAuxiliarEntitiesTo.toLocalDateTime(fechaStart)
        val end = convertersAuxiliarEntitiesTo.toLocalDateTime(fechaEnd)

        assertThat(end).isBefore(start)
    }
    @Test
    fun `Dataset d1 contiene 2 identifiers`() {
