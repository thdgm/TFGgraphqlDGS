...
suspend fun getDataServiceInfo(id_dserv: String?,page: Int): Collection<DatasetSeriesInfo> {
    ...
    if (id_dserv != null){
        seriesInfo =  apolloClient.query(DatasetInfoQuery(id= id_dserv!!, isDataset = false, isCatalog = false, page = page,isDataService = false, isDatasetSeries = true)).execute().data?.resource?.onDatasetSeries
    }
