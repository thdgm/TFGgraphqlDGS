...
@SpringBootTest
class TestFindResourcesSameFormat {
    ...
    @Test
    fun `Creo CRs a partir de json y obtengo los de un formato concreto`() {
        ...
        val queryParameters = mutableMapOf<String, Any>("filter" to "format", "value" to listOf("application/json"))
    ...
    fun `Creo CRs a partir de json y obtengo los de un set de formatos concreto`() {
        ...
        val queryParameters = mutableMapOf<String, Any>("filter" to "format", "value" to listOf("application/json","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
        val resources = dgsQueryExecutor.executeAndGetDocumentContext(query, queryParameters)
        val response = GraphQLResponse(resources.jsonString())
        LoggerFactory.getLogger("loggerTest").debug("RESPUESTAAAAA:  $response")

        val resourcesId = response.extractValue<Collection<String>>("data.resourcesByFilter[*].id")
        //LoggerFactory.getLogger("loggerTest").debug("SIZE:  ${resourcesId.size}")
        Assertions.assertThat(resourcesId).hasSize(8)
