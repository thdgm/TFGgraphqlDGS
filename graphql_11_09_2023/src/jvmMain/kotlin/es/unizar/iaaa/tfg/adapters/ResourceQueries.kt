...
class ResourceQueries(
    ...
    @DgsQuery
    fun resource(@InputArgument id: String?,@InputArgument isDataset: Boolean,@InputArgument isCatalog: Boolean,@InputArgument isDataService: Boolean,@InputArgument isDatasetSeries: Boolean,@InputArgument page: Int): Resource? {
        if (id == null) {
            return null
        }
        println("RESSSSSSSS")
    ...
    fun inCatalog(filter: String?, dfe: DgsDataFetchingEnvironment): Collection<Catalog?> {
        val res: Resource = dfe.getSource()
        println("LETSSSSSS1")
        val v = resourceServices.getCatalogOfResource(filter, res.id)
        println("LETSSSSSS1 FIN")
        return v
    }
