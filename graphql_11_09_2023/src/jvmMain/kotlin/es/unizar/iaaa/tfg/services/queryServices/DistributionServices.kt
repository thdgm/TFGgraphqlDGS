...
interface DistributionServices {
    ...
    fun getByteSize(id: String): NonNegativeDouble?
...
class DistributionServicesImpl(
    ...
    override fun getDistribution(id: String): Distribution? {
        val c = distributionRepository.findbyDistId(id)//.findById(id)
        return if (c != null) converter.toDistribution(c) else null
    }
    ...
    override fun getDistributionAccessService(id: String): Collection<DataService?> =
        dataServiceRepository.findAccessServiceByDistributionsId(id)
            .map {
                converter.toDataServiceById(it)
            }.toList()

    // Return accessService field of Distribution id
    override fun getDistributionAccessUrl(id: String): String? =
        distributionRepository.findByIdOrNull(id)?.accessUrl


    // Return byteSize distribution id
    override fun getByteSize(id: String): NonNegativeDouble? =
        NonNegativeDouble(distributionRepository.findbyDistId(id)?.byteSize ?: 0.0)
    ...
    override fun getDatasets(id: String): Collection<Dataset> =
        datasetRepository.findDatasetsByDistributionsDatasetId(id, PageRequest.of(0,5)).mapNotNull { converter.toDataset(it) }
