...
class ResourcesByFilterServicesImpl(
    ...
    @Cacheable
    override fun getDatasetsByFilters(appliedFilters: List<MapInput>, type: String,page: Int,sortBy: String, orderBy: String?): Collection<Resource>{
    ...
    @Cacheable
    override fun getCatalogsByFilters(appliedFilters: List<MapInput>, type: String,page: Int, sortBy: String, orderBy: String?): Collection<Resource>{
        //Conversiones de issued, modified y period
    ...
    @Cacheable
    override fun getServicesByFilters(appliedFilters: List<MapInput>, type: String,page: Int, sortBy: String, orderBy: String?): Collection<Resource>{

        // Formatear como cadena en el nuevo formato
        val issued = appliedFilters.find { it.key == "Fecha creación"}?.values?.map{converterAux.toLocalDateTime(it)} ?: listOf()
        val modified = appliedFilters.find { it.key == "Fecha última modificación"}?.values?.map{converterAux.toLocalDateTime(it)} ?: listOf()
        println("FECHASSSSS:::::::::::::::::::::::::::: $issued ----- $modified")
        val notation = appliedFilters.find { it.key == "Nivel de administración"}?.values?.map{"${converterAux.getAdministrationLevel(it)}%"} ?: listOf()
        var numberOfResources: String? = if (appliedFilters.find { it.key == "Número de recursos"}?.values?.isNotEmpty() == true) appliedFilters.find { it.key == "Número de recursos"}?.values?.first() else null
        var rangeNumber: Pair<String, String>? = null
        if (numberOfResources != null){
    ...
    @Cacheable
    override fun getSeriesByFilters(appliedFilters: List<MapInput>, type: String,page: Int, sortBy: String, orderBy: String?): Collection<Resource>{
        val issued = appliedFilters.find { it.key == "Fecha creación"}?.values?.map{converterAux.toLocalDateTime(it)} ?: listOf()
        val modified = appliedFilters.find { it.key == "Fecha última modificación"}?.values?.map{converterAux.toLocalDateTime(it)} ?: listOf()
        val notation = appliedFilters.find { it.key == "Nivel de administración"}?.values?.map{"${converterAux.getAdministrationLevel(it)}%"} ?: listOf()
    ...
    override fun getResourcesByFilters(filters: Collection<MapInput>, type: String,page: Int): Collection<Resource>{
        ...
        println("GEETTTTTT RESOURCESSSSSSSSSSSSSSSSSSSSSS -------------------------------------------------------------------$appliedFilters ------- $type")
        sortBy = when(sortBy){
            "issued" -> {"issued"}
            "modified" -> {"modified"}
    ...
    //Cast for example: 2023-09-08T12:58:50+27:50 to 2023-09-08 12:58:50.273336
    fun castLocalDteTime(date: String): String{
        val dateTimePart = date.substring(0, 19)
        // Extraer la parte de la zona horaria y ajustarla (agregar 27 horas y 50 minutos)
        val hours = date.substring(20, 22).toLong()
        val minutes = date.substring(23, 25).toLong()
        val adjustedDateTime = LocalDateTime.parse(dateTimePart, DateTimeFormatter.ISO_LOCAL_DATE_TIME)
            .plusHours(hours)
            .plusMinutes(minutes)
        // Agregar milisegundos (en este caso, 273336 milisegundos)
        var adjustedWithMillis =  adjustedDateTime.withNano(273336 * 1000)
        return adjustedWithMillis.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS"))
    }
}
