...
class CreateRelationsBetweenEntitiesServicesImpl(
    ...
    override fun insertIntoThemesResources(themeIds: Collection<String?>, resourceId: String){
        println("------------------------------------------------------------------------------------------------------------------------------")
        println("INSERTOOOOOOOOO THEMESSSSSSS:::::: $resourceId")
        ...
        themeIds.forEach {
          if (!it.isNullOrBlank()){
              if (!themeRepository.existsById(it)) {
                  ...
                  newTheme.id = it
                  //if (!themeRepository.existsById(it))
                  if (!themeRepository.existsById(it)) themeRepository.save(newTheme)
                  println("INSERTOOOOOOOOO THEMESSSSSSS2:::::: $it --- $resourceId")
              }
          }
      }
