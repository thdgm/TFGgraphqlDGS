...
suspend fun getResourceInfo(id_dataset: String?): Collection<DatasetInfo> {
    ...
    if (id_dataset != null){
        datasetInfo =  apolloClient.query(DatasetInfoQuery(id= id_dataset!!,isDataset = true, isCatalog = false, page = 0, isDataService = false, isDatasetSeries = false)).execute().data?.resource?.onDataset
    }
    ...
    if (datasetInfo != null) {
        ...
        return listOf(
            DatasetInfo(
                ...
                if (!datasetInfo.publisher?.notation.isNullOrBlank()) ADMON_LEVEL[datasetInfo.publisher!!.notation[0]] else null,
                ...
                datasetInfo.distributions?.map{ DistInfo(it.accessUrl, it.id) },
                datasetInfo.isPrimaryTopicOf?.map{it.id} ?: emptyList(),
                datasetInfo.inCatalog?.map{it.id} ?: emptyList(),
                datasetInfo.inSeries?.map{it.id} ?: emptyList(),
                datasetInfo.isServedBy?.map{it.id} ?: emptyList(),
...
/*suspend fun getDatasets(filter: String?, values: List<String>?): List<DatasetModel>{
    val l =  apolloClient.query(DatasetsQuery(filter=Optional.present(filter), type=Optional.present(values), page = 0)).execute().data?.resourcesByFilter?.filterNotNull() ?: emptyList()
