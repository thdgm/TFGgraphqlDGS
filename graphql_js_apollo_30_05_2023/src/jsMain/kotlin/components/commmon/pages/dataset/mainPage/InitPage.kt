...
suspend fun getResourcesNumber(resType:String): Int {
    val number =  apolloClient.query(NumberOfResourcesQuery(type=resType)).execute().data?.numberOfResources
    return number ?: 0
}
...
val InitPage = FC<InitPageProps> { props->
    ...
    useEffect(numberDatasets) {
        GlobalScope.launch {
            console.log("TOTAL NUMBER")
            numberDatasets = getResourcesNumber("dataset")
        }
