...
interface PublisherRepository : JpaRepository<PublisherEntity, String>{
    ...
    @Query(
        value =
        "select * from \"publisher\" as t WHERE t.\"notation\" is not null and t.\"notation\" <> ''",
        nativeQuery = true
    )
    @Transactional
    fun findAllNotationNotNull(pageable: Pageable): Page<PublisherEntity>

    @Query(
        value =
        "select * from \"publisher\" as t WHERE t.\"label\" is not null and t.\"notation\" <> ''",
        nativeQuery = true
    )
    @Transactional
    fun findAllLabelNotNull(pageable: Pageable): Page<PublisherEntity>
}
