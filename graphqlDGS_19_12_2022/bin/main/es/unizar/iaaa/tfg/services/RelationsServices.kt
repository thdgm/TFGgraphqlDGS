...
class InitBasicServicesId(
    ...
    override fun showCatalogRecord(id: String): CatalogRecords? {
        return catalogRecordsRepository.findById(id).get().toCatalogRecords()
    }

    //FUNCION AUXILIAR
    fun createResource(tipo:String,id:String,title:String):ResourceInCatalog{
        return when(tipo){
            "catalog"-> Catalog(id,title)
            "data_service" -> DataService(id,title)
            "dataset"-> Dataset(id,title)
            "dataset_series"-> DatasetSeries(id,title)
            else -> throw IllegalArgumentException("Tipo desconocido $tipo")
        }
    }
    fun createDataset(tipo:String,id:String,title:String):DatasetInCatalog{
        return when(tipo){
            "catalog"-> Catalog(id,title)
            "dataset"-> Dataset(id,title)
            "dataset_series"-> DatasetSeries(id,title)
