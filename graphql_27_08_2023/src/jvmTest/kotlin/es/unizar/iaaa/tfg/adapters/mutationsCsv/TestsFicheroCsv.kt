...
class TestsFicheroCsv {
    ...
    val urlComplete = "classpath:datosgobes.csv"
    ...
    @Autowired
    lateinit var convertersResourcesEntitiesTo: ConvertersResourcesEntitiesTo
    ...
    @DirtiesContext(methodMode = DirtiesContext.MethodMode.AFTER_METHOD)
    @Test
    fun `Creo CR todos los del csv`() {

        val inputParam = "\$input"
        var i = 0

        processCsvServices.processCsvService(urlComplete).map {
           println("Create: $i")
            i++
        }
    ...
    fun `Creo CRs a partir de json y obtengo los de un admon level concreto`() {
        ...
        query getResourcesByAdmonLevel($filterParam:String, $valueParam:[String!]){
            resourcesByFilter(filters:$filterParam, type:$valueParam, page: 0){
                id
                publisher
    ...
    fun `Check resourcesByFilter if it has null parameter`() {
        ...
        val query = """
        query Datasets($filterParam:String, $valueParam:[String!], $page:Int!) {
            resourcesByFilter (filters:$filterParam, type: $valueParam, page:$page) {
                ... on Dataset{
                    id
                    title
                    publisher
        ...
        //assertThat(relatedResources).hasSize(2)
        val res = resourceRepository.findAll().map { convertersResourcesEntitiesTo.createResource(it) }
        getLogger("loggerTest").debug("RESPUESTAAAAA2:  $res")
        val res2 = resourceRepository.findAll().map { it}
        getLogger("loggerTest").debug("RESPUESTAAAAA3:  $res2")
        val res3 = resourceRepository.findAllDatasets(PageRequest.of(0,6)).content.map { it}
        getLogger("loggerTest").debug("RESPUESTAAAAA4:  $res3")
    ...
    fun `Creo CRs a partir de json y obtengo todos los themes por rangos`() {
        ...
        println(nvc)
*/
        var t = themeRepository.findAll(PageRequest.of(0,6)).content
            .map { if (it == null) "NULL" else it.id }.distinct()
        println(t)

        t = themeRepository.findAllIdIsNotNull(PageRequest.of(0,6)).content
            .map { if (it == null) "NULL" else it.id }.distinct()
        println(t)

        t = themeRepository.findAllIdIsNotNull(PageRequest.of(page,if(pageSize >= 0) pageSize else Integer.MAX_VALUE)).content
            .map { it.id }.distinct()
        println(t)

        /*var dist =  distributionRepository.findAll(PageRequest.of(0,6)).content.mapNotNull { it.format?.let { it1 -> MediaType.parseMediaType(it1).subtype } }
            .distinct()
        println(dist)

        dist = distributionRepository.findAllFormatIsNotNull(PageRequest.of(0,6)).content.mapNotNull { it.format?.let { it1 -> MediaType.parseMediaType(it1).subtype } }
            .distinct()
        println(dist)

        var key =  keywordRepository.findAll().map { it.id.wordId }.distinct()
        println(key)
        key =  keywordRepository.findAllWordIsNotNull(PageRequest.of(0,6)).map { it.id.wordId }.distinct()
        println(key)*/
    ...
    @DirtiesContext(methodMode = DirtiesContext.MethodMode.AFTER_METHOD)
    @Test
    fun `Obtengo resources por filtro y paginación`() {
        ...
        val query = """
        query Datasets($filterParam:String, $valueParam:[String!],$pageParam: Int!) {
            resourcesByFilter (filters:$filterParam, type: $valueParam, page: $pageParam) {
                ... on Dataset{
                    id
                    title
                    publisher
    ...
    fun `GetAllAdminLevel`() {
        ...
        val query = """
        query GetAll($pageParam: Int!,$pageSizeParam: Int!) {
            getAllThemes(page:$pageParam, pageSize:$pageSizeParam)
        ...
        //val queryParameters = mutableMapOf<String, Any?>("filter" to null, "value" to listOf<String>())
        val resources = dgsQueryExecutor.executeAndGetDocumentContext(query, queryParameters as Map<String, Any>)
        val response = GraphQLResponse(resources.jsonString())
        getLogger("loggerTest").debug("RESPUESTAAAAA:  $response")
