...
class TestsFicheroCsv {
    ...
    fun keywordsAndLanguagePT(datasetModel: String) {
        ...
        val titles = response.extractValue<Collection<String>>("data.createCatalogRecord.primaryTopic.title[*]")
        ...
        println(languages)
        println(keywords.distinct())
        println(titles.distinct())
    ...
    fun `El CR tiene primaryTopic a un dataset compruebo su title keywords y language`() {
        processCsvServices.processCsvService(urlRecord).map{
            keywordsAndLanguagePT(it.toString())
        }
    }
    ...
    @DirtiesContext(methodMode = DirtiesContext.MethodMode.AFTER_METHOD)
    @Test
    fun `Creo CR a partir de json y su dataset tiene un publisher`() {
        val inputParam = "\$input"
        val contentUrlEsperado = "classpath:json1.json"
        val contentTypeEsperado = "application/json"
        val hintsEsperado = listOf("datos.gob.es")
        ...
        assertThat(publisherNotation).isEqualTo("E00003801")
        assertThat(publisherLabel).isEqualTo("Ministerio del Interior")
    }
