...
suspend fun getAllContentTypes(more: Boolean): Collection<String>{
    if(!more){
        return apolloClient.query(ContentTypeQuery(page = 0, pageSize = -1)).execute().data?.getAllContentTypes?.filterNotNull() as MutableCollection<String>? ?: mutableListOf()
    }
    return apolloClient.query(ContentTypeQuery(page = 0, pageSize = 6)).execute().data?.getAllContentTypes?.filterNotNull() as MutableCollection<String>? ?: mutableListOf()
}
suspend fun getAllHints(more: Boolean): Collection<String>{
    if(!more){
        return apolloClient.query(HintsQuery(page = 0, pageSize = -1)).execute().data?.getAllHints?.filterNotNull() as MutableCollection<String>? ?: mutableListOf()
    }
    return apolloClient.query(HintsQuery(page = 0, pageSize = 6)).execute().data?.getAllHints?.filterNotNull() as MutableCollection<String>? ?: mutableListOf()
}
