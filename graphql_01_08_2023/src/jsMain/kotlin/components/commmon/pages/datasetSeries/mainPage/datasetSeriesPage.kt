...
suspend fun getSeries(filters: MutableMap<String, MutableMap<String, Collection<String>>>, typeRes: String, page: Int): List<SeriesModel>{
    ...
    val l =  components.commmon.pages.catalog.mainPage.apolloClient.query(DatasetsQuery(filter= Optional.present(filter), type=typeRes, page = page, isDataset = false, isCatalog = false)).execute().data?.resourcesByFilter?.filterNotNull() ?: emptyList()
    val ll =  l.map{
        if (it.onCatalog != null) {
            ...
            SeriesModel(it.onCatalog.id, title, description,it.onCatalog.issued.toString(),it.onCatalog.modified.toString(), it.onCatalog.numberOfResources ?: 0)
        }else{
            null
        }
    }.filterNotNull()
