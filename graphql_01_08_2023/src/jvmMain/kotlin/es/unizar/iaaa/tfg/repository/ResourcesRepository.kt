...
println("NUMBERSSSSSSSSSSSSSSSSS")
        ...
        var queryString = "SELECT COUNT(DISTINCT result.id) as total_elements " +
                ...
                "    ) as tt " +
                "    WHERE tt.tipo = 'catalog' AND "
                //"    GROUP BY tt.id, tt.tipo, tt.issued, tt.license, tt.modified, tt.publisher_id, tt.accrual_periodicity, tt.temporal_coverage_end, tt.temporal_coverage_start, tt.validity " +
               // "    HAVING COUNT(tt.resources) BETWEEN 0 AND 100 " +
                //") as result;"
...
interface ResourceRepository : JpaRepository<ResourceEntity, String> {
   ...
   /* @Query(
    ...
    fun findDistinctServesDatasetByDatasetServiceId(id: String): Collection<ResourceEntity>
