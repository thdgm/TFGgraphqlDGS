...
interface LanguageRepository : JpaRepository<LanguageEntity, String> {
    ...
    fun findLanguagesResourcesByDescriptionsText(id: String): Collection<LanguageEntity>
    fun findLanguagesResourcesByTitlesTitle(id: String): Collection<LanguageEntity>
...
interface DescriptionRepository : JpaRepository<ResourceDescriptionsEntity, String> {
    ...
    @Modifying
    @Query(
        value = "INSERT INTO \"languages_descriptions\" (\"id_language\",\"id_description\") VALUES (?1,?2)",
        nativeQuery = true
    )
    @Transactional
    fun insertInLanguageDescriptions(languageId: String, descriptionId: String)
}
...
interface TitleRepository : JpaRepository<TitlesEntity, String> {
    ...
    @Modifying
    @Query(
        value = "INSERT INTO \"languages_titles\" (\"id_language\",\"id_title\") VALUES (?1,?2)",
        nativeQuery = true
    )
    @Transactional
    fun insertInLanguageTitles(languageId: String, titleId: String)
}
