...
class CatalogRepositoryTest {
    ...
    fun `el catalogo1 tiene 3 datasets`() {
        ...
        getLogger("loggerTest").debug("$catalog")

        assertEquals(3, catalog.resourcesCatalog.filter { it.type != ConstantValues.DATASERVICETYPE }.size)
    }
    ...
    fun `el catalogo1 tiene 1 service`() {
        val catalog = catalogRepository.findByIdOrNull("catalog1") ?: fail("El catalogo no existe")
        assertEquals(1, catalog.resourcesCatalog.filter { it.type == ConstantValues.DATASERVICETYPE }.size)
    }

    @Test
    fun `el catalogo1 tiene 1 catalog`() {
        val catalog = catalogRepository.findByIdOrNull("catalog1") ?: fail("El catalogo no existe")
        assertEquals(1, catalog.resourcesCatalog.filter { it.type == ConstantValues.CATALOGTYPE }.size)
    }
