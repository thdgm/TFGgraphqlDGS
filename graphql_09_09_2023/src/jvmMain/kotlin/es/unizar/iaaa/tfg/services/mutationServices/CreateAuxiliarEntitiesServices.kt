...
interface CreateAuxiliarEntitiesServices {
    ...
    fun createKeywordsFromCsv(keywords: Map<String, String>, idRes:String,res: ResourceEntity)
    ...
    fun createResourceDescriptionsFromCsv(descriptions: Collection<String>, datasetId: String,res: ResourceEntity)
...
class CreateAuxiliarEntitiesServicesImpl(
        ...
        keywords.keys.map {
            ...
            keywords[it]?.split("//")?.map{
               ...
               if(!languageRepository.existsById(language)){
                ...
                newKeyword.language = newLanguage//languageRepository.findById(language).get()
                languagesList.add(newKeyword.language)
                keywordRepository.save(newKeyword)
            }
        }
        createRelationsBetweenEntitiesLanguageServices.insertIntoLanguagesResources(languagesList, res/*resourceRepository.findById(idRes).get()*/)
        ...
        title.map {
            when(res) {
                ...
                is DistributionEntity -> {
                    ...
                    println("TITLE: $langTitle")
                    if(!langTitle.elementAt(1).isNullOrBlank()) {
                        titleId.titleId = langTitle.elementAt(1)
                        titleId.distributionId = res.id
                        tit.id = titleId
                        val language = langTitle.elementAt(0)
                        if(!titlesDistributionRepository.existsById(tit.id)) titlesDistributionRepository.insertInTitlesDist(res.id,langTitle.elementAt(1))//.save(tit)
                        if (!language.isNullOrBlank()) {
                            val newLang = LanguageEntity()
                            if (!languageRepository.existsById(language)){
                                newLang.id = language
    ...
    override fun createLocationFromCsv(spatial: String): MutableCollection<LocationEntity> {
        var locationsList = mutableListOf<LocationEntity>()
        spatial.split("//").map{
            val newLocation = LocationEntity()
            newLocation.nombre = it
    ...
    override fun createResourceDescriptionsFromCsv(descriptions: Collection<String>, datasetId: String, res: ResourceEntity) {
        val languagesList = mutableListOf<LanguageEntity>()
        descriptions.forEach {
            val langText = it.split("[[LANG2]]")
            ...
            languagesList.add(newLanguage/*languageRepository.findById(newLanguage.id).get()*/)
        ...
        createRelationsBetweenEntitiesLanguageServices.insertIntoLanguagesResources(languagesList, res/*resourceRepository.findById(datasetId).get()*/)
    }
    ...
    override fun createRegulations(regulations:Collection<String>, dataset:DatasetEntity){
        regulations.forEach {
            if(!it.isNullOrBlank()) {
                ...
                newRegul.id = it
                if (!regulationsRepository.existsById(it)) regulationsRepository.save(newRegul)
    ...
    override fun createRelatedResources(resRelated:Collection<String>, dataset:DatasetEntity){
        println("RELATEDD: $resRelated")
        resRelated.map {
            if(!it.isNullOrBlank()) {
                val newResRel = RelatedResourcesEntity()
                newResRel.id = it
                if(!relatedResourcesRepository.existsById(it)) relatedResourcesRepository.save(newResRel)
