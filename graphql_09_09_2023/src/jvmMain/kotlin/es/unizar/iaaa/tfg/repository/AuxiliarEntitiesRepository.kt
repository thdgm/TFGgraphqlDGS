...
interface DescriptionRepository : JpaRepository<ResourceDescriptionsEntity, String> {
    ...
    @Query(
        value = "select id_description, id_language from languages_descriptions where id_resource = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findDescriptionsByResourceId(id: String): Collection<Any?>
...
interface TitleDistributionRepository : JpaRepository<TitlesDistributionEntity, String> {
    ...
    fun existsById(id: TitleDistributionId): Boolean
    ...
    @Query(
        value =
        "INSERT INTO \"titles_distribution\" (\"id_distribution\",\"id_title\") " +
                "VALUES (?1,?2)",
        nativeQuery = true
    )
    @Transactional
    fun insertInTitlesDist(distributionId: String, titleId: String)
...
interface KeywordRepository : JpaRepository<KeywordEntity, String> {
    @Query(
        value = "select * from keyword where id_dataset = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findKeywordsByDatasetId(id: String): Collection<KeywordEntity> //CHANGE

    @Query(
        value = "select k.id_word from (select distinct id_word, COUNT(id_word) from keyword WHERE id_word IS NOT NULL GROUP BY id_word ORDER BY COUNT(id_word) DESC LIMIT 5) as k",
        nativeQuery = true
    )
    @Transactional
...
interface LocationRepository : JpaRepository<LocationEntity, String> {
    @Query(
        value = "select location_id from dataset_location where dataset_id = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findLocationsByDatasetsId(id: String): Collection<String>//CHANGE
...
interface ThemeRepository : JpaRepository<ThemeEntity, String>{
    @Query(
        value = "select id_theme from themes_resources where id_resource = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findThemeByResourcesId(id: String):Collection<String?>//CHANGE
    @Modifying
    @Query(
...
interface RegulationsRepository : JpaRepository<RegulationsEntity, String>{
    @Query(
        value = "select id_regulations from regulations_resources where id_dataset = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findRegulationsByDatasetsId(id: String):Collection<String?>//CHANGE
    @Modifying
    @Query(
...
interface RelatedResourcesRepository : JpaRepository<RelatedResourcesEntity, String>{
    @Query(
        value = "select id_related_resources from resources_related where id_dataset = ?1",
        nativeQuery = true
    )
    @Transactional
    fun findRelatedResourcesByDatasetsId(id: String):Collection<String?>//CHANGEE
    @Modifying
    @Query(
