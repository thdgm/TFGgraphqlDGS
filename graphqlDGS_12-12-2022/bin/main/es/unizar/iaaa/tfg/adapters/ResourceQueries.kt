...
class ResourceQueries(
    ...
    fun resource(@InputArgument id: String?): Resource? {
        ...
        return initService.showResourceInCatalog(id) as Resource? /*showResourcesInCatalog().firstOrNull {
            when (it) {
                is Dataset -> it.id == id
                is Catalog -> it.id == id
    ...
    fun resources(@InputArgument id: String?): ResourceInCatalog? {
        ...
        return initService.showResourceInCatalog(id)/*showResourcesInCatalog().firstOrNull {
            when (it) {
                is Dataset -> it.id == id
                is Catalog -> it.id == id
                is DatasetSeries -> it.id == id
