...
class DistributionQueries(
    ...
    fun title(dfe: DgsDataFetchingEnvironment): Collection<LangString?> {
        ...
        println("TIIIIIITTTTLTLLLLLLLLEEEEEEEEESSSSSSSSSsSSSSSS ${dist!!.id} --  ${distributionServices.getDistributionTitles(dist!!.id)}")

        return emptyList()//distributionServices.getDistributionTitles(dist!!.id)
    }
