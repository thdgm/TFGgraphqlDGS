...
class CreateAuxiliarEntitiesServicesImpl(
    ...
    override fun createKeywords(jsonFields: MutableMap<ModelJsonMapping, String>): Collection<KeywordEntity> {
        ...
        println("KEYYYYYYYYYYWWWWOOOOORORRRRRRRDDDDDDDDDSSSSSSSSSSSSSSSS1")
        if(keywordWords != null){
            keywordWords.forEachIndexed { index, word ->
                ...
                kwId.datasetId = datasetModel.id
                kw.id = kwId
                println("KEYYYYYYYYYYWWWWOOOOORORRRRRRRDDDDDDDDDSSSSSSSSSSSSSSSS2")
                kw.language = languageRepository.findById(keywordLangs.elementAt(index)!!).get()
                //keywords.add(kw)
                println("KEYYYYYYYYYYWWWWOOOOORORRRRRRRDDDDDDDDDSSSSSSSSSSSSSSSS3")
                keywordRepository.save(kw)
                println("KEYYYYYYYYYYWWWWOOOOORORRRRRRRDDDDDDDDDSSSSSSSSSSSSSSSS4")
                keywords.add(kw)
            }
        }
        println("KEYYYYYYYYYYWWWWOOOOORORRRRRRRDDDDDDDDDSSSSSSSSSSSSSSSS5")
        ...
        if (titleTexts.isNotEmpty()) {
            titleTexts.forEachIndexed { index, text ->
                if (res is DistributionEntity){
                    println("kjsabdkjabdkjsabdkjsabdkjsabdksabdksajbdkjasbdkasjbdksajdksbdksabdkasbkasbdksbdkasbdkasbdkasbdkasbdksabdsabdkasbdkasbdksabdksabdjasbdjkasbdkasbdskabdkasb 0")
                    val tit = TitlesDistributionEntity()
                    println("kjsabdkjabdkjsabdkjsabdkjsabdksabdksajbdkjasbdkasjbdksajdksbdksabdkasbkasbdksbdkasbdkasbdkasbdkasbdksabdsabdkasbdkasbdksabdksabdjasbdjkasbdkasbdskabdkasb 1 $text")
                    ...
                    println("kjsabdkjabdkjsabdkjsabdkjsabdksabdksajbdkjasbdkasjbdksajdksbdksabdkasbkasbdksbdkasbdkasbdkasbdkasbdksabdsabdkasbdkasbdksabdksabdjasbdjkasbdkasbdskabdkasb 2")
                    println("INNNNNSEEEEEEEEEEEEEEEEEEEEERRRRRRRRRRRRRRRRRTTTTTTTTTTTTTTTTOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                    println("${tit.id.titleId} -- ${tit.id.distributionId}")
                   ...
                   println("INNNNNSEEEEEEEEEEEEEEEEEEEEERRRRRRRRRRRRRRRRRTTTTTTTTTTTTTTTTOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                    println("${tit.id.titleId} -- ${tit.id.resourceId}")
                    titlesResourceRepository.save(tit)

                    createRelationsBetweenEntitiesServices.insertIntoLanguageTitles(language,tit.id.titleId,res)
                }

                /*if (!titlesRepository.existsById(tit.id.titleId)) {
                    println("CREATE DISTRIBUTIONS TITLLLLEEEEEE1 ${ tit.distributionTitle.id}")
                    println("CREATE DISTRIBUTIONS TITLLLLEEEEEE2 ${tit}")
                    titlesRepository.save(tit)
                }*/
               // createRelationsBetweenEntitiesServices.insertIntoLanguageTitles(language,tit.id.titleId)
            }
        }
    }
    ...
    override fun createResourceDescriptions(
        ...
        if (descTexts.isNotEmpty()) {
            descTexts.forEachIndexed { index, text ->
                ...
                //if (!descriptionRepository.existsById(descId)) {
                    println("METO ESTO: ${desc.id.descriptionId}")
                    println("METO ESTO: $language")
