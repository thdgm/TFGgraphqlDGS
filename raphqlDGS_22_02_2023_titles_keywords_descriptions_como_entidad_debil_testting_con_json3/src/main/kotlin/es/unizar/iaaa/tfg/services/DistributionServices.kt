...
class DistributionServicesImpl(
    ...
    override fun getDistributionTitles(id: String): Collection<LangString?> {
        println("INICIO $id")
        println(titleDistributionRepository.findByDistributionTitleId(id))
        ...
        val l = titleDistributionRepository.findByDistributionTitleId(id)
            .map {
                ...
                //languageStrings.add(converterAuxiliar.toLangString(title.id.titleId + LANGSTRING_SEPARADOR + it.language.id))
                             ...
                             println("TIITTIIITTIIITTIIIES: ${it.id.titleId} ---- ${languageRepository.findLanguagesResourcesByTitlesDistributionIdTitleId(it.id.titleId)}")
                           languageRepository.findLanguagesResourcesByTitlesDistributionIdTitleId(it.id.titleId)
                                 .forEach {
                                     println("LANGSTRING: ${title.id.titleId} -- ${it.id}")

                                     languageStrings.add(converterAuxiliar.toLangString(title.id.titleId + LANGSTRING_SEPARADOR + it.id))
                                     println("LANGSTRING2: $languageStrings")
                                 }
                languageStrings
            }.map{
        ...
        println(titleDistributionRepository.findByDistributionTitleId(id).map { it.id.titleId })
        val titles = l.elementAt(0)
        //println("TIIIIIITTTTLTLLLLLLLLEEEEEEEEESSSSSSSSSsSSSSSS ${l.elementAt(0).map { it?.tag }}")
