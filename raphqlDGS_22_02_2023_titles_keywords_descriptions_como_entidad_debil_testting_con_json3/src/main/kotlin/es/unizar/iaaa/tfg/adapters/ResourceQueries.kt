...
class ResourceQueries(
    ...
    fun title(dfe: DgsDataFetchingEnvironment): Collection<LangString?> {
        ...
        println("TIIIIIITTTTLTLLLLLLLLEEEEEEEEESSSSSSSSSsSSSSSS ${res!!.id} --  ${resourceServices.getTitles(res!!.id)}")
    ...
    // @DgsQuery identifier: returns the identifier field of Resource
    @DgsData(parentType = "Resource")
    fun identifier(dfe: DgsDataFetchingEnvironment): String? {
        val res: Resource? = dfe.getSource()
        return resourceServices.getIdentifier(res!!.id)
    }
